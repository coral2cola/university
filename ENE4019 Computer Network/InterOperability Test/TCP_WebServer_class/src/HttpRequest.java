   ...
   // final static int BUFFER_OUT_SIZE = 1024;
    ...
    private void sendResponseMessage(FileInputStream fis, DataOutputStream os) throws Exception {
    	...
    	if(code != StatusCode.OK)
    		entityBody = entityBody.replaceAll("\\?", message + " - sent by YeonJu's WebServer");
    ...
    private void sendHeaderLines(DataOutputStream os) throws Exception{
    	...
    	switch (code) {
    	case OK:
    		contentLength += "1024" + CRLF + contentTypeLine + contentType + CRLF;
    		
    		break;
    	default:
    		contentLength += "1024" + CRLF + contentTypeLine + CONTENT_TYPES.getProperty("html") + CRLF;
    	}
	...
	private String parseRequestLine(String requestLine)
	{
		...
		String method = tokens.nextToken().toUpperCase(); //GET
		String fileName = tokens.nextToken(); // /index.html
		String version = tokens.nextToken().toUpperCase(); //HTTP/1.1
		...
		if(file.isDirectory())
		{
			...
			{
				code = StatusCode.NOT_FOUND;
				return "No index file found at reqeusted location " + fileName + " Returning NOT FOUND";
			}
