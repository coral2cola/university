...
def render():
    ...
    M[:3,:3] = Rz @ Ry @ Rx
    """
    xang = np.radians(alpha)
    yang = np.radians(beta)
    zang = np.radians(gamma)
    M = np.identity(4)
    
    Rx = np.array([[1,0,0],
                   [0, np.cos(yang), -np.sin(yang)],
                   [0, np.sin(yang), np.cos(yang)]])
    Ral = np.array([[np.cos(xang), -np.sin(xang), 0],
                   [np.sin(xang), np.cos(xang), 0],
                   [0,0,1]])
    Rgam = np.array([[np.cos(zang), -np.sin(zang), 0],
                   [np.sin(zang), np.cos(zang), 0],
                   [0,0,1]])
    M[:3,:3] = Ral @ Rx @ Rgam
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018008531-8-1', None,None)
    if not window:
        glfw.terminate()
        return
